...
problem = pycollo.OptimalControlProblem(state_variables=[y0, y1, y2, y3], control_variables=[u0, u1], parameter_variables=[m0, p0], initial_mesh=mesh)
...
# Bounds
problem.bounds = pycollo.Bounds(optimal_control_problem=problem, initial_time=0, final_time=[1, 3], state=[[-np.pi, np.pi], [-np.pi, np.pi], [-10, 10], [-10, 10]], control=[[-15, 15], [-15, 15]], parameter=[[0.5, 1.5], [0.5, 1.5]], integral=[0, 1000], state_endpoint=[[-np.pi/2, -np.pi/2], [-np.pi/2, -np.pi/2], [0, 0], [0, 0], [np.pi/2, np.pi/2], [np.pi/2, np.pi/2], [0, 0], [0, 0]])

# Guess
problem.initial_guess = pycollo.Guess(optimal_control_problem=problem, time=[0, 2], state=[[-np.pi/2, np.pi/2], [-np.pi/2, np.pi/2], [0, 0], [0, 0]], control=[[0, 0], [0, 0]], integral=[100], state_endpoints_override=True)

# Auxiliary data
problem.auxiliary_data = dict({g: -9.81, d0: 0.5, k0: 1/12, m1: 1.0, p1: 0.5, d1: 0.5, k1: 1/12, l0: p0 + d0, l1: p1 + d1, I0: m0*(k0**2 + p0**2), I1: m1*(k1**2 + p1**2), c0: sym.cos(y0), s0: sym.sin(y0), c1: sym.cos(y1), s1: sym.sin(y1), M00: I0 + m1*l0**2, M01: m1*p1*l0*(s0*s1 + c0*c1), M10: M01, M11: I1, K0: u0 + g*(m0*p0 + m1*l0)*c0 + m1*p1*l0*(s1*c0 - s0*c1)*y3**2, K1: u1 + g*m1*p1*c1 + m1*p1*l0*(s0*c1 - s1*c0)*y2**2, detM: M00*M11 - M01*M10})
