...
if not scale:
	...
	mesh = pycollo.Mesh(mesh_sections=num_mesh_secs, mesh_section_fractions=None, mesh_collocation_points=mesh_col_points)
	...
	# Integrand functions
	problem.integrand_functions = [225*(u0**2)]
	...
	# Bounds
	problem.bounds = pycollo.Bounds(optimal_control_problem=problem, initial_time=0, final_time=final_time, state=[[-np.pi, np.pi], [-10, 10]], control=[[-0.5, 0.5]], integral=[0, 1000], state_endpoint=[[-np.pi/2, -np.pi/2], [0, 0], [np.pi/2, np.pi/2], [0, 0]])
	...
	# Auxiliary data
	problem.auxiliary_data = {g: -9.81, m0: 1.0, p0: 0.5, d0: 0.5, k0: 1/12, T0min: -15, T0max: 15}
...
if scale:
	...
	# Optimal Control Problem
	mesh = pycollo.Mesh(mesh_sections=num_mesh_secs, mesh_section_fractions=None, mesh_collocation_points=mesh_col_points)
	problem = pycollo.OptimalControlProblem(state_variables=[y0, y1], control_variables=[u0, q0], initial_mesh=mesh)

	# State equations
	problem.state_equations = [dphi0/phi0sigma, (gm0p0c0plusT0)/(I0*dphi0sigma)]

	# Integrand functions
	problem.integrand_functions = [q0minusT0T0]
	...
	# Bounds
	problem.bounds = pycollo.Bounds(optimal_control_problem=problem, initial_time=0, final_time=final_time, state=[[-0.5, 0.5], [-0.5, 0.5]], control=[[-0.5, 0.5], [0, 200]], integral=[0, 200], state_endpoint=[[-0.25, -0.25], [0, 0], [0.25, 0.25], [0, 0]])

	# Guess
	problem.initial_guess = pycollo.Guess(optimal_control_problem=problem, time=[0, final_time], state=[[-0.25, 0.25], [0, 0]], control=[[0, 0], [0, 0]], integral=[10], state_endpoints_override=True)

	# Auxiliary data
	# problem.auxiliary_data = {g: -9.81, m0: 1.0, p0: 0.5, d0: 0.5, k0: 1/12, l0: p0 + d0, I0: m0*(k0**2 + p0**2), c0: sym.cos(y0)}
	problem.auxiliary_data = dict({g: -9.81, m0: 1.0, p0: 0.5, d0: 0.5, k0: 1/12, l0: p0 + d0, I0: m0*(k0**2 + p0**2), phi0mu: (phi0min + phi0max)/2, phi0sigma: phi0max - phi0min, dphi0mu: (dphi0min + dphi0max)/2, dphi0sigma: dphi0max - dphi0min, T0mu: (T0min + T0max)/2, T0sigma: T0max - T0min, phi0: phi0mu + y0*phi0sigma, dphi0: dphi0mu + y1*dphi0sigma, T0: T0mu + u0*T0sigma, c0: sym.cos(phi0), phi0min: -np.pi, phi0max: np.pi, dphi0min: -10, dphi0max: 10, T0min: -15, T0max: 15, q0min: 0, q0max: 200, gm0: g*m0, p0c0: p0*c0, gm0p0c0: gm0*p0c0, gm0p0c0plusT0: gm0p0c0 + T0, T0T0: T0**2, q0minusT0T0: q0 - T0T0})
