...
mesh = col.Mesh(mesh_sections=1000, mesh_section_fractions=None, mesh_collocation_points=7)
...
# Bounds
problem.bounds = col.Bounds(initial_time_lower=0.0, initial_time_upper=1.0, final_time_lower=1.0, final_time_upper=2.0, state=[['-inf', 'inf'], ['-inf', 'inf']], control_lower='-inf', control_upper='inf', integral_lower=[0], integral_upper=['inf'], parameter=[[1.0, 2.0], [1.0, 2.0]], boundary_lower=[0, 0, np.pi, 0], boundary_upper=[0, 0, np.pi, 0])

# Guess
problem.initial_guess = col.Guess(time=np.array([0.0, 1.0]), state=np.array([[0.0, np.pi], [0.0, 0.0]]), control=np.array([[0.0, 0.0]]), integral=[1000], parameter=[1.5, 1.5])
