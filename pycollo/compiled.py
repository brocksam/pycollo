...
INTEGRAL = "integral"
COLLOCATION_MATRIX_FORMS = Options(
    (DIFFERENTIAL, INTEGRAL), default=INTEGRAL, unsupported=DIFFERENTIAL
)
...
class CompiledFunctions:
    ...
    def compile_objective_gradient(self):
        ...
        for p, p_slice in zip(
            self.ocp_backend.p, self.ocp_backend.phase_endpoint_variable_slices
        ):
    ...
    def compile_constraints(self):
        ...
        for p in self.ocp_backend.p:
            ...
            t_norm_dependent_tiers = getattr(
                expr_graph, f"t_norm_P{p.i}_dependent_tiers"
            )
        ...
        c_continuous_lambdas = []
        for p, p_slice in zip(
            self.ocp_backend.p, self.ocp_backend.phase_constraint_slices
        ):
        ...
        for p in self.ocp_backend.p:
            dstretch_dt = np.array(
                [
                    val
                    for val, t_needed in zip(
                        np.array([-0.5, 0.5]), p.ocp_phase.bounds._t_needed
                    )
                    if t_needed
                ]
            )
    ...
    def compile_jacobian_constraints(self):
        def phase_jacobian_lambda(
            ...
            dzeta_ds = dc_dx[phase_c_defect_slice, phase_t_slice.stop :, :].reshape(
                -1, N
            )
            dgamma_dy = dc_dx[phase_c_path_slice, phase_y_slice, :].reshape(-1, N)
            dgamma_du = dc_dx[phase_c_path_slice, phase_u_slice, :].reshape(-1, N)
            dgamma_ds = dc_dx[phase_c_path_slice, phase_t_slice.stop :, :].reshape(
                -1, N
            )
            ...
            drho_ds = dc_dx[phase_c_integral_slice, phase_t_slice.stop :, :].reshape(
                -1, N
            )

            G_dzeta_dy = phase_G_dzeta_dy_lambda(dzeta_dy, stretch, A, D, ocp_num_y)
            G_dzeta_du = (
                phase_G_dzeta_du_lambda(dzeta_du, stretch, A, ocp_num_y, ocp_num_u)
                if num_u
                else None
            )
            G_dzeta_dq = None
            G_dzeta_dt = (
                phase_G_dzeta_dt_lambda(
            ...
            G_dzeta_ds = (
                phase_G_dzeta_ds_lambda(
                    dzeta_ds, stretch, A, ocp_num_y, num_c_defect, num_s
                )
                if num_s
                else sparse.csr_matrix((num_c_defect, num_s))
            )
            G_dgamma_dy = (
                phase_G_dgamma_dy_lambda(dgamma_dy, ocp_num_c_path, ocp_num_y)
                if num_c_path
                else None
            )
            G_dgamma_du = (
                phase_G_dgamma_du_lambda(dgamma_du, ocp_num_c_path, ocp_num_u)
                if (num_c_path and num_u)
                else None
            )
            G_dgamma_dq = None
            G_dgamma_dt = None
            G_dgamma_ds = (
                phase_G_dgamma_ds_lambda(dgamma_ds, num_c_path, num_s)
                if (num_c_path and num_s)
                else sparse.csr_matrix((num_c_path, num_s))
            )
            G_drho_dy = (
                phase_G_drho_dy_lambda(drho_dy, stretch, W, num_c_integral, num_y)
                if num_c_integral
                else None
            )
            G_drho_du = (
                phase_G_drho_du_lambda(drho_du, stretch, W, num_c_integral, num_u)
                if (num_c_integral and num_u)
                else None
            )
            G_drho_dq = (
                phase_G_drho_dq_lambda(num_q) if (num_c_integral and num_q) else None
            )
            G_drho_dt = (
                phase_G_drho_dt_lambda(g, dstretch_dt, W, num_c_integral, num_t)
                if (num_c_integral and num_t)
                else None
            )
            G_drho_ds = (
                phase_G_drho_ds_lambda(drho_ds, stretch, W, num_c_integral, num_s)
                if (num_c_integral and num_s)
                else sparse.csr_matrix((num_c_integral, num_s))
            )
        ...
        def phase_G_dzeta_dy_lambda(ddy_dy, stretch, A, D, ocp_num_y):
            ...
            G_dzeta_dy = sparse.bmat(
                np.array(G_dzeta_dy).reshape(ocp_num_y, ocp_num_y), dtype=float
            )
        ...
        def phase_G_drho_ds_lambda(drho_ds, stretch, W, num_c_integral, num_s):
            G_drho_ds = sparse.csr_matrix(
                (-stretch * np.matmul(drho_ds, W)).reshape(
                    (num_c_integral), num_s, order="F"
                )
            )
            return G_drho_ds
        ...
        def endpoint_jacobian_lambda(
            ...
            for db_dxb_lambda, N, num_y, num_q, num_t, num_x in zip(
                ...
                col_indices = np.tile(
                    np.concatenate([y_row_indices, q_row_indices]), num_c_endpoint
                )
        ...
        for p, p_x_slice, p_c_slice in zip(
            ...
            dc_dx_nodes_parameter = dc_dx_nodes[
                p_c_slice, self.ocp_backend.variable_slice
            ]
            dc_dx_nodes_joined = (
                np.hstack([dc_dx_nodes_phase, dc_dx_nodes_parameter]).flatten().tolist()
            )
        ...
        for p, p_slice in zip(
            ...
            db_dxb_phase = sym.Matrix(sym.SparseMatrix(*db_dxb.shape, db_dxb.entries))[
                :, p_slice
            ]
    ...
    def compile_hessian_lagrangian(self):
        ...
        def continuous_hessian_lambda(H_shape, x_tuple, lagrange, H_indices_and_flags):
            ...
            for data, ((root_row_index, root_col_index), flag) in zip(
                phase_data, H_indices_and_flags
            ):
                if flag == 1:
        ...
        def hessian_lambda(
            ...
            H += continuous_hessian_lambda(
                H_shape, x_tuple, lagrange, H_continuous_indices
            )
            return H
