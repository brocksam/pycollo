...
INTEGRAL = "integral"
COLLOCATION_MATRIX_FORMS = Options((DIFFERENTIAL, INTEGRAL),
                                   default=INTEGRAL,
...
class CompiledFunctions:
    ...
    def compile_reshape(self):
        def reshape_x(x, y_slices, u_slices, q_slices, t_slices, s_slice, p_N):
            x = np.array(x)
            x_phase_tuples = []
            for y_slice, u_slice, q_slice, t_slice, N in zip(
                    y_slices, u_slices, q_slices, t_slices, p_N):
    ...
    def compile_objective_gradient(self):
        ...
        for p, p_slice in zip(self.ocp_backend.p,
                              self.ocp_backend.phase_endpoint_variable_slices):
    ...
    def compile_constraints(self):
        ...
        for p in self.ocp_backend.p:
            ...
            t_norm_precomputable = getattr(expr_graph,
                                           f"t_norm_P{p.i}_precomputable")
            t_norm_dependent_tiers = getattr(expr_graph,
                                             f"t_norm_P{p.i}_dependent_tiers")
        ...
        c_continuous_lambdas = []
        for p, p_slice in zip(self.ocp_backend.p,
                              self.ocp_backend.phase_constraint_slices):
        ...
        for p in self.ocp_backend.p:
            dstretch_dt = np.array([
                val for val, t_needed in zip(np.array(
                    [-0.5, 0.5]), p.ocp_phase.bounds._t_needed) if t_needed
            ])
    ...
    def compile_jacobian_constraints(self):
        def phase_jacobian_lambda(
                ...
                phase_q_slice,
                phase_t_slice,
                phase_c_defect_slice,
                phase_c_path_slice,
                phase_c_integral_slice,
        ):
            ...
            dzeta_dy = dc_dx[phase_c_defect_slice, phase_y_slice, :].reshape(
                -1, N)
            dzeta_du = dc_dx[phase_c_defect_slice, phase_u_slice, :].reshape(
                -1, N)
            dzeta_ds = dc_dx[phase_c_defect_slice, phase_t_slice.
                             stop:, :].reshape(-1, N)
            dgamma_dy = dc_dx[phase_c_path_slice, phase_y_slice, :].reshape(
                -1, N)
            dgamma_du = dc_dx[phase_c_path_slice, phase_u_slice, :].reshape(
                -1, N)
            dgamma_ds = dc_dx[phase_c_path_slice, phase_t_slice.
                              stop:, :].reshape(-1, N)
            drho_dy = dc_dx[phase_c_integral_slice, phase_y_slice, :].reshape(
                -1, N)
            drho_du = dc_dx[phase_c_integral_slice, phase_u_slice, :].reshape(
                -1, N)
            drho_ds = dc_dx[phase_c_integral_slice, phase_t_slice.
                            stop:, :].reshape(-1, N)

            G_dzeta_dy = phase_G_dzeta_dy_lambda(dzeta_dy, stretch, A, D,
                                                 ocp_num_y)
            ...
            G_dzeta_dt = (phase_G_dzeta_dt_lambda(
                dy, A, dstretch_dt, ocp_num_y, num_c_defect, num_t)
                          if num_t else None)
            G_dzeta_ds = (phase_G_dzeta_ds_lambda(
                dzeta_ds, stretch, A, ocp_num_y, num_c_defect, num_s) if num_s
                          else sparse.csr_matrix((num_c_defect, num_s)))
            G_dgamma_dy = (phase_G_dgamma_dy_lambda(
                dgamma_dy, ocp_num_c_path, ocp_num_y) if num_c_path else None)
            G_dgamma_du = (phase_G_dgamma_du_lambda(dgamma_du, ocp_num_c_path,
                                                    ocp_num_u) if
                           (num_c_path and num_u) else None)
            G_dgamma_dq = None
            G_dgamma_dt = None
            G_dgamma_ds = (phase_G_dgamma_ds_lambda(dgamma_ds, num_c_path,
                                                    num_s) if
                           (num_c_path and num_s) else sparse.csr_matrix(
                               (num_c_path, num_s)))
            G_drho_dy = (phase_G_drho_dy_lambda(drho_dy, stretch, W,
                                                num_c_integral, num_y)
                         if num_c_integral else None)
            G_drho_du = (phase_G_drho_du_lambda(drho_du, stretch, W,
                                                num_c_integral, num_u) if
                         (num_c_integral and num_u) else None)
            G_drho_dq = (phase_G_drho_dq_lambda(num_q) if
                         (num_c_integral and num_q) else None)
            G_drho_dt = (phase_G_drho_dt_lambda(g, dstretch_dt, W,
                                                num_c_integral, num_t) if
                         (num_c_integral and num_t) else None)
            G_drho_ds = (phase_G_drho_ds_lambda(drho_ds, stretch, W,
                                                num_c_integral, num_s) if
                         (num_c_integral and num_s) else sparse.csr_matrix(
                             (num_c_integral, num_s)))
            ...
            G_parameter = sparse.vstack([G_dzeta_ds, G_dgamma_ds,
                                         G_drho_ds]).tocoo()

            phase_data = G_phase.data
            phase_row = G_phase.row + phase_row_offset
            phase_col = G_phase.col + phase_col_offset
            G_phase = sparse.coo_matrix((phase_data, (phase_row, phase_col)),
                                        shape=G_shape)
            ...
            G_parameter = sparse.coo_matrix(
                (parameter_data, (parameter_row, parameter_col)),
                shape=G_shape)
        ...
        def phase_G_dzeta_dt_lambda(dy, A, dstretch_dt, ocp_num_y,
                                    num_c_defect, num_t):
        ...
        def phase_G_dzeta_ds_lambda(ddy_ds, stretch, A, ocp_num_y,
                                    num_c_defect, num_s):
        ...
        def phase_G_drho_dt_lambda(g, dstretch_dt, W, num_c_integral, num_t):
            G_drho_dt = sparse.csr_matrix(
                -np.outer(dstretch_dt, np.matmul(g, W)).T)
            return G_drho_dt

        def phase_G_drho_ds_lambda(drho_ds, stretch, W, num_c_integral, num_s):
            G_drho_ds = sparse.csr_matrix(
                (-stretch * np.matmul(drho_ds, W)).reshape((num_c_integral),
                                                           num_s,
                                                           order="F"))
            return G_drho_ds

        def endpoint_jacobian_lambda(
                G_shape,
                x_point_tuple,
            ...
            row_offset = 0
            db_dxb_phases = []
            for db_dxb_lambda, N, num_y, num_q, num_t, num_x in zip(
                    db_dxb_phase_lambdas,
                    p_N,
                    num_y_per_phase,
                    num_q_per_phase,
                ...
                y_row_indices = np.array([
                    (i * N, (i + 1) * N - 1) for i in range(ocp_num_y)
                ]).flatten()
                q_row_indices = np.array(range(num_x - num_q - num_t, num_x))
                row_indices = np.repeat(np.array(range(num_c_endpoint)),
                                        ocp_num_y * 2 + num_q + num_t)
                col_indices = np.tile(
                    np.concatenate([y_row_indices, q_row_indices]),
                    num_c_endpoint)
                db_dxb_phase = sparse.coo_matrix(
                    (db_dxb_phase, (row_indices, col_indices)),
                    shape=(num_c_endpoint, num_x),
                )
                db_dxb_phases.append(db_dxb_phase)
            db_ds = sparse.csr_matrix(
                db_ds_lambda(*x_point_tuple).reshape(num_c_endpoint, num_s))
        ...
        def jacobian_lambda(
                ...
                ocp_phase_p_slice,
                ocp_phase_g_slice,
                ocp_phase_c_defect_slice,
                ocp_phase_c_path_slice,
                ocp_phase_c_integral_slice,
                num_y_per_phase,
                num_u_per_phase,
                num_q_per_phase,
                num_t_per_phase,
                num_x_per_phase,
                num_s,
                num_c_defect_per_phase,
                num_c_path_per_phase,
                num_c_integral_per_phase,
                num_c_per_phase,
                num_c_endpoint,
                phase_y_slices,
                phase_u_slices,
                phase_q_slices,
                phase_t_slices,
                phase_c_defect_slices,
                phase_c_path_slices,
                phase_c_integral_slices,
        ):
            ...
            for (
                    ...
                    self._t_stretch_lambdas,
                    self._t_dstretch_dt_lambdas,
                    self.c_continuous_lambdas,
                    dc_dx_lambdas,
                    p_A,
                    p_D,
                    p_W,
        ...
        dc_dx_lambdas = []
        for p, p_x_slice, p_c_slice in zip(
                self.ocp_backend.p,
            ...
            dc_dx_nodes_parameter = dc_dx_nodes[p_c_slice, self.ocp_backend.
                                                variable_slice]
        ...
        for p, p_slice in zip(self.ocp_backend.p,
            ...
            db_dxb_phase = sym.Matrix(
                sym.SparseMatrix(*db_dxb.shape, db_dxb.entries))[:, p_slice]
        ...
        db_dxb = expr_graph.db_dxb
        db_ds = sym.Matrix(sym.SparseMatrix(
            *db_dxb.shape,
            db_dxb.entries))[:, self.ocp_backend.endpoint_variable_slice]
    ...
    def compile_hessian_lagrangian(self):
        def endpoint_hessian_lambda(H_shape, x_point_tuple, sigma, lagrange,
                                    H_indices):
            data = ddL_dxbdxb_lambda(*x_point_tuple, sigma, *lagrange)
            H = sparse.coo_matrix((data, H_indices), shape=H_shape)
            return H

        def continuous_hessian_lambda(H_shape, x_tuple, lagrange,
                                      H_indices_and_flags):
            phase_data = ddL_dxdx_lambda(*x_tuple, *lagrange)
            H = sparse.dok_matrix(H_shape)
            for data, ((root_row_index, root_col_index),
                       flag) in zip(phase_data, H_indices_and_flags):
                if flag == 1:
        ...
        def hessian_lambda(
                ...
                sigma,
                lagrange,
                H_continuous_indices,
                H_endpoint_indices,
        ):
            H = endpoint_hessian_lambda(H_shape, x_point_tuple, sigma,
                                        lagrange, H_endpoint_indices)
            H += continuous_hessian_lambda(H_shape, x_tuple, lagrange,
                                           H_continuous_indices)
            return H
