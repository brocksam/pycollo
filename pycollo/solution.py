...
class Solution:
    ...
    def _plot_interpolated_solution(self, plot_y=False, plot_dy=False, plot_u=False):
        ...
        for y_polys, dy_polys, u_polys, p_data, mesh_index_boundaries in zip(self._phase_y_polys, self._phase_dy_polys, self._phase_u_polys, self._phase_data, self._it.mesh.mesh_index_boundaries):
            ...
            t_start_stops = list(zip(
                p_data.tau[mesh_index_boundaries[:-1]], p_data.tau[mesh_index_boundaries[1:]]))
        ...
        if plot_y:
            for p, p_data, t_data, y_datas in zip(self._backend.p, self._phase_data, t_data_phases, y_datas_phases):
                for i_y, y_data in enumerate(y_datas):
                    plt.plot(
                        p_data.time, p_data.y[i_y], marker='x', markersize=5, linestyle='', color='black')
                    plt.plot(t_data, y_data, linewidth=2,
                             label=str(p.y_vars[i_y])[1:])
        ...
        if plot_dy:
            for p, p_data, t_data, dy_datas in zip(self._backend.p, self._phase_data, t_data_phases, dy_datas_phases):
                for i_y, dy_data in enumerate(dy_datas):
                    plt.plot(
                        p_data.time, p_data.dy[i_y], marker='x', markersize=5, linestyle='', color='black')
        ...
        if plot_u:
            for p, p_data, t_data, u_datas in zip(self._backend.p, self._phase_data, t_data_phases, u_datas_phases):
                for i_u, u_data in enumerate(u_datas):
                    plt.plot(
                        p_data.time, p_data.u[i_u], marker='x', markersize=5, linestyle='', color='black')
                    plt.plot(t_data, u_data, linewidth=2,
                             label=str(p.u_vars[i_u])[1:])
            plt.legend(loc='upper left')
            plt.grid(True)
            plt.title('Controls')
            plt.xlabel('Time / $s$')
        ...
        if False:
            ...
            with open('result_tradjectory.csv', mode='w') as result_tradjectory_file:
                ...
                result_tradjectory_writer = csv.writer(
                    result_tradjectory_file, delimiter=',')
        ...
        if False:
            ...
            ax1.plot(self._time, self._y[0], marker='x',
                     markersize=7, linestyle='', color='black')
            ...
            ax2.set_ylabel(
                'Angular Velocity / $rad \\cdot s^{-1}$', color=color)
            ax2.plot(self._time, self._dy[0], marker='x',
                     markersize=7, linestyle='', color='black')
            ...
            plt.plot(self._time, self._u[0] * 15, marker='x',
                     markersize=7, linestyle='', color='black')
